...
class DllPlugin {
	...
	apply(compiler) {
		compiler.hooks.entryOption.tap("DllPlugin", (context, entry) => {
			const itemToPlugin = (item, name, options) => {
				if (Array.isArray(item)) {
					return new DllEntryPlugin(context, item, name, options);
				}
				throw new Error("DllPlugin: supply an Array as entry");
			};
			if (typeof entry === "object" && !Array.isArray(entry)) {
				Object.keys(entry).forEach(name => {
					itemToPlugin(entry[name], name, this.options).apply(compiler);
				});
			} else {
				itemToPlugin(entry, "main", false).apply(compiler);
			}
			return true;
		});
